...
export default function EmployeeTickets() {
  ...
  const fetchTickets = () => {
    fetch(`/api/tickets?all=true&status=${filter}&assigned=me`)
      .then(r => r.json())
  ...
  return (
    <>
      ...
      <div className="pg-sub">Handle client support requests assigned to you.</div>
        ...
        <div className="card">
          ...
          {loading ? (
            ...
            <div className="tbl-wrap">
              <table>
                <thead><tr><th>Ticket#</th><th>Client</th><th>Title</th><th>Priority</th><th>Status</th><th>Updated</th><th></th></tr></thead>
                <tbody>
                  {tickets.length === 0
                    ? <tr><td colSpan={7} style={{ textAlign: 'center', color: '#999', padding: 32 }}>No tickets found.</td></tr>
                    : tickets.map((t) => (
                      <tr key={t.id}>
        ...
        <div className="card">
          <div className="card-header">
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              {selected.status === 'OPEN' && <button className="btn btn-primary btn-sm" onClick={() => updateStatus('IN_PROGRESS')}>Start Working</button>}
              {selected.status === 'IN_PROGRESS' && <button className="btn btn-success btn-sm" onClick={() => updateStatus('RESOLVED')}>Mark Resolved</button>}
